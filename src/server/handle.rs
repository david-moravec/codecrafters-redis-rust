        ...
        loop {
            tokio::select! {
                ...
                propagated_frame = self.repl_frame_propagation.recv() => {
                    ...
                    eprintln!("[propagation:replica@{:?}] {:?}",   self.connection.stream.get_ref().peer_addr().unwrap().port(),frame);
                    self.connection.write_frame(&frame).await?;
                ...
                server_cmd = self.server_command_propagation.recv() => {
                    ...
                    eprintln!("[to   :replica@{:?}] {:?}",   self.connection.stream.get_ref().peer_addr().unwrap().port(),server_cmd.cmd);
                    self.connection.write_frame(&server_cmd.cmd).await?;
                    let response = self.connection.read_frame().await?;
                    eprintln!("[from :replica@{:?}] {:?}",   self.connection.stream.get_ref().peer_addr().unwrap().port(),response);
                ...
                .ok_or(anyhow!("query channel closed"))?
            {
                ReplicationCommand::Wait {
                    ...
                    if replica_count == 0 {
                        ...
                        eprintln!("replica handles recv count {:}", recv_count);
